...
to see if the Q-function converges. We take $\Delta t$ as 10000.
...
\begin{figure}
  ...
  \begin{subfigure}{\textwidth}
    ...
    % \label{fig:sub2}
  \end{subfigure}
  \caption{Plots of Q-learning showing the convergence}
  % \label{fig:test}
\end{figure}
...
We also plot the rolling average delta Q-function $|Q^{(t)} - Q^{(t-\Delta t)}|$ over time
to see if the Q-function converges. The $\Delta t$ is 10000 and the size of the rolling window
is 20. We use the rolling mean in log scale plot since the sudden change is
small in normal scale but large in log scale
...
\begin{figure}
  ...
  \begin{subfigure}{\textwidth}
    ...
    \caption{The rolling average delta Q-function $|Q^{(t)} - Q^{(t-\Delta t)}|$ over time in log scale}
    % \label{fig:sub2}
  \end{subfigure}
