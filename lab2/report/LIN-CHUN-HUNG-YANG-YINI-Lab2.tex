...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[1]
    ...
    \For{each episode $k=1,2,\cdots$}
      \For{$t=0, \cdots, N$}
        \State Get the $\epsilon$-greedy action $a_t=\pi(s_t)$ according to $Q_{\phi}(s_t,a_t)$ \Comment{Line 270}
        \State Obtain observations $(s_t,a_t,r_t,s_{t+1})$    \Comment{Line 271-272}
        \State Add $(s_t,a_t,r_t,s_{t+1})$ to the memeory set \Comment{Line 275}
        \For{$i=1,\cdots,|B|$}  \Comment{In agent.train\_model}
          ...
          \State Calculate the target value:
            ...
            \end{equation*}
          \State Update $\phi$:
          $$\phi\leftarrow\phi-\alpha(Q_{\phi}(s_t,a_t)-y_i)\nabla_{\phi}Q_{\phi}(s_t,a_t)$$ \Comment{Line: 172, self.model.fit(...)}
        \EndFor
      \EndFor
